...
# --------- build flavor marker ---------
# Generate this only in the WSL build workspace.  The source checkout stays
# flavor-neutral, while the packaged app can hide diagnostic-only screens in a
# release build without relying on Android environment variables.
BUILD_INFO_PATH="$WORKSPACE/reversible_mosaic/_build_info.py"
cat > "$BUILD_INFO_PATH" <<PYTHON
"""Build-time flavor information generated by wsl_build_android.sh."""

BUILD_FLAVOR = "$BUILD_MODE"
SHOW_STAGE0_SELF_TEST = $([ "$BUILD_MODE" = "debug" ] && printf 'True' || printf 'False')
