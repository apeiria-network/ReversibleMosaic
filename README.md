...
打开 **ReversibleMosaic** 后，会看到首页。下图左侧是首页，右侧是进入 **图片打码** 后的操作页面，便于对照整个开始流程：

![APP 首页与打码页面](docs/README/app-title-and-encrypted.jpg)
...
在首页点击 **图片打码**，然后按以下步骤操作（上方组合图右侧展示了该页面）：
...
在首页点击 **图片恢复**，选择之前生成的混淆图片。下图左侧是图片恢复页面，右侧是恢复成功后的结果页面：

![APP 恢复与恢复结果页面](docs/README/app-restored-and-result.jpg)
...
恢复完成后，APP 会在上方组合图右侧显示恢复出的图片。确认结果无误后，可以将图片保存到手机；也可以使用手机上其他应用自行传递已保存的图片和分享代码。
